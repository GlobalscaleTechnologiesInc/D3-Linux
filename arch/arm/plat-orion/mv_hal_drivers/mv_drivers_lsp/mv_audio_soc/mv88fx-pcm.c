...
struct mv88fx_snd_chip *mv88fx_pcm_snd_chip[2];
				    ...
				    struct mbus_dram_target_info *dram)
{
	...
	if (chip->pdata->i2s_play || chip->pdata->spdif_play)
		if (mv88fx_config_dma_window(chip, SNDRV_PCM_STREAM_PLAYBACK,
			dram) < 0)
			return -1;

	if (chip->pdata->i2s_rec || chip->pdata->spdif_rec)
		if (mv88fx_config_dma_window(chip, SNDRV_PCM_STREAM_CAPTURE,
			dram) < 0)
			return -1;
	return 0;
}
...
static irqreturn_t mv88fx_pcm_dma_interrupt(int irq, void *dev_id)
{
	...
	struct snd_pcm_substream *play_stream = NULL;
	struct snd_pcm_substream *capture_stream = NULL;
					       ...
					       *machine_data)
{
	...
	mv88fx_snd_debug("");
	chip = mv88fx_pcm_snd_chip[machine_data->port] =
	...
	if (chip->pdata->i2s_play || chip->pdata->spdif_play)
		mv88fx_pcm_init_stream(chip,
			&chip->stream[SNDRV_PCM_STREAM_PLAYBACK],
			SNDRV_PCM_STREAM_PLAYBACK);
	if (chip->pdata->i2s_rec || chip->pdata->spdif_rec)
		mv88fx_pcm_init_stream(chip,
			&chip->stream[SNDRV_PCM_STREAM_CAPTURE],
...
error:
	...
	kfree(chip);
	mv88fx_pcm_snd_chip[machine_data->port] = NULL;
	return NULL;
}

static int mv88fx_pcm_new(struct snd_soc_pcm_runtime *rtd)
{
...
static void mv88fx_pcm_free(struct snd_pcm *pcm)
{
	...
	free_irq(mv88fx_pcm_snd_chip[pdev->id]->irq, mv88fx_pcm_snd_chip[pdev->id]);
	kfree(mv88fx_pcm_snd_chip[pdev->id]);
	mv88fx_pcm_snd_chip[pdev->id] = NULL;
}
...
struct snd_soc_platform_driver mv88fx_soc_platform = {
...
static int mv88fx_pcm_probe(struct platform_device *pdev)
{
	return snd_soc_register_platform(&pdev->dev, &mv88fx_soc_platform);
}
static int mv88fx_pcm_remove(struct platform_device *pdev)
{
	snd_soc_unregister_platform(&pdev->dev);
}

static struct platform_driver mv88fx_pcm_driver = {
	.probe = mv88fx_pcm_probe,
	.remove = __devexit_p(mv88fx_pcm_remove),
	.driver = {
...
{
	return platform_driver_register(&mv88fx_pcm_driver);
}
module_init(mv88fx_soc_platform_init);

static void __exit mv88fx_soc_platform_exit(void)
{
	platform_driver_unregister(&mv88fx_pcm_driver);
}
