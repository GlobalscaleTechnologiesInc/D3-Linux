...
#if 1
static void  ak_clk_enable(struct clk *clk)
{  /*software do nothing, hardware already enabled. */ }
static void  ak_clk_disable(struct clk *clk)
{  /*software do nothing, hardware already enabled. */ }   
static int ak_clk_setrate(struct clk *clk, unsigned long rate)
{  *(clk->rate) = rate;  return 0;  }
static unsigned long ak_clk_getrate(struct clk *clk)
{  return *(clk->rate);  }
const struct clkops akosc_clk_ops = {
	.enable		= ak_clk_enable,
	.disable	= ak_clk_disable,
	.setrate	= ak_clk_setrate,
	.getrate	= ak_clk_getrate,
};
static unsigned long akosc_rate = 0;
static struct clk akosc_clk = {
	.ops	= &akosc_clk_ops,
	.rate	= &akosc_rate,
};
#endif
...
static struct clk_lookup dove_clocks[] = {
	...
	INIT_CK("sdhci-dove.0", NULL, &clk_sdio0),
	INIT_CK("sdhci-dove.1", NULL, &clk_sdio1),
...
//	INIT_CK(NULL, "GIGA_PHY", &clk_giga_phy),
	...
	INIT_CK(NULL, "AKOSC_CLK", &akosc_clk),
};
