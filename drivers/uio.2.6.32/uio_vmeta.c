...
int vmeta_release(struct uio_info *info, struct inode *inode)
{
	...
	vmeta_print(" *** vmeta_release() is called!\n");

	for(current_id=0;current_id<MAX_VMETA_INSTANCE;current_id++) {
		if(current->tgid == ks->user_id_list[current_id].pid) {
			printk(KERN_ERR "vmeta release current tgid(%d) pid(%d), instance id=%d, active id=%d\n",\
...
static int vmeta_ioctl(struct uio_info *info, unsigned int cmd, unsigned long arg)
{
	...
	//vmeta_print("  *** vmeta-->vmeta_ioctl:: cmd=0x%x\n",cmd);
	switch(cmd) {
...
static int vmeta_probe(struct platform_device *pdev)
{
	...
	if (!mem_vir_addr) {
		printk(KERN_ERR "vmeta_probe: can't alloc DMA  VDEC_HW_CONTEXT_SIZE,\n");
		ret = -ENOMEM;
		goto out_free;
	}
	...
	if (!mem_vir_addr) {
		printk(KERN_ERR "vmeta_probe: can't alloc DMA  VDEC_OBJ_SIZE,\n");
		ret = -ENOMEM;
		goto out_free;
	}
	...
	if (!mem_vir_addr) {
		printk(KERN_ERR "vmeta_probe: can't alloc DMA  KERNEL_SHARE_SIZE,\n");
		ret = -ENOMEM;
		goto out_free;
	}
